...
|---|---|---|
| `bench_provision_progress(bench_name)` | While a bench is provisioning (after `release_group_create_deploy_candidate`). Returns a single dict with the current `stage` (build / new_bench / setup_bench / site_migrate / ready / failed) plus the full chain. **Replaces the broken pattern of polling the filesystem** — directories are empty for 5-10 min during setup_bench by design. | low |
| `list_pending_releases(app?, release_group?, app_source?, limit?)` returns `{count, filters, releases:[{name, app, source, hash, status, creation}, ...]}`. Note: no `tag` field — Press's App Release schema doesn't have one. | List Draft App Releases waiting for approval. | low |
| `site_update(name)` wraps `press.api.site.update` with a pre-flight: if no newer candidate exists, returns `{ok:false, reason:'no_destination_candidate', hint:...}` instead of throwing the misleading "Could not find suitable Destination Bench" error. Always check `ok` before assuming the call queued the update. | Migrate a site to the latest candidate. | high |
