<template>
	<div class="space-y-4">
			...
			<div v-for="app in appScores" :key="app.app" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
				<div class="mb-2 flex items-center justify-between">
					<div class="flex items-center gap-1.5">
						...
						<a v-if="getAppRepo(app.app)" :href="'https://github.com/' + getAppRepo(app.app)" target="_blank" @click.stop class="text-gray-400 hover:text-blue-500" title="Open on GitHub">
							<svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 16 16"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>
						</a>
					</div>
					<Badge size="sm" :label="app.scores.overall + '%'" :theme="app.scores.overall >= 80 ? 'green' : app.scores.overall >= 50 ? 'orange' : 'red'" />
				</div>
		...
		<div v-show="activeTab === 'stack' && !scanning" class="grid grid-cols-3 gap-4">
			<div v-for="app in stackInfo" :key="app.app" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
				...
				<div class="grid grid-cols-2 gap-1 text-xs">
					...
					<a v-if="app.repository" :href="'https://github.com/' + app.repository" target="_blank" @click.stop
						class="text-blue-500 truncate hover:text-blue-400 hover:underline">
						{{ app.repository }} ↗
					</a>
				</div>
			</div>
		...
		<!-- TAB: Code Graph + Deep Analysis (extracted to child component) -->
		<HealthAdvanced v-if="(activeTab === 'codegraph' || activeTab === 'deep') && !scanning"
			:tab="activeTab" :bench-name="benchName" :apps="graphApps" :deep-apps="deepApps" />
...
export default {
	...
	methods: {
		...
		getAppRepo(appName) {
			const s = this.stackInfo.find(a => a.app === appName);
			return s?.repository || '';
		},
		toggleFilter(f) {
			f.active = !f.active;
